		...
		'<li class="ss-uploadfield-item template-download{% if (file.error) { %} ui-state-error{% } %}" data-fileid="{%=file.id%}">' + 
			...
			'{% } %}' +
				...
				'<label class="ss-uploadfield-item-url">' + 
					'<span class="url" title="{%=file.url%}">{%=file.url%}</span>' +
					'<div class="clear"><!-- --></div>' + 
				'</label>' +
				'{% if (file.error) { %}' +
					'<div class="ss-uploadfield-item-actions">' + 
						'<div class="ss-uploadfield-item-cancel ss-uploadfield-item-cancelfailed delete"><button class="icon icon-16 btn btn-danger" data-icon="delete" title="' + ss.i18n._t('UploadField.CANCELREMOVE', 'Cancel/Remove') + '">' + ss.i18n._t('UploadField.CANCELREMOVE', 'Cancel/Remove') + '</button></div>' +
					'</div>' +
				'{% } else { %}' +
